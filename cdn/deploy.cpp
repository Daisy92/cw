...
void deploy_server(char * topo[MAX_EDGE_NUM], int line_num, char * filename)
{
    ...
    check_srcs(g, network_info, customer_info, of);
     ...
     immune<Input_File_Info, Input_File_Info,_EVAL> teach(network_info, customer_info,total_cost);
     teach.imCreateGene();
     gettime(current_ms,current_s);
      while(compare_time())
	{
		teach.imSendSpecies();
		teach.imAfinity(super_ford_fulkerson(am, s, g.target));
		teach.imDensity();
